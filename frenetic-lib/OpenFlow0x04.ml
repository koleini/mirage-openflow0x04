...
module PacketIn = struct
  ...
  let parse (bits : Cstruct.t) : t =
    ...
    let final_bits = get_new_buffer total_len in
  ...
  let parse (bits : Cstruct.t) : t =
    ...
    let po_payload = match bufId with
      | None -> 
        ...
        let final_bits = get_new_buffer (Cstruct.len data_bits) in
        Cstruct.blit data_bits 0 final_bits 0 (Cstruct.len data_bits);
        NotBuffered final_bits
      | Some n -> 
        let final_bits = get_new_buffer 0 in
...
module TableFeature = struct
    ...
    let marshal (buf : Cstruct.t) (tf : t) : int =
      ...
      set_ofp_table_features_pad (Cstruct.to_string (get_new_buffer 5)) 0 buf;
...
module FlowStats = struct
  ...
  let marshal (buf : Cstruct.t) (fs : flowStats) : int =
    ...
    set_ofp_flow_stats_pad1 (Cstruct.to_string (get_new_buffer 4)) 0 buf;
...
module AggregateStats = struct
  ...
  let marshal (buf : Cstruct.t) (ag : aggregStats) : int =
    ...
    set_ofp_aggregate_stats_reply_pad (Cstruct.to_string (get_new_buffer 4)) 0 buf;
    sizeof_ofp_aggregate_stats_reply
...
module TableStats = struct
  ...
  let marshal (buf : Cstruct.t) (ts : tableStats) : int =
    ...
    set_ofp_table_stats_pad (Cstruct.to_string (get_new_buffer 3)) 0 buf;
...
module PortStats = struct
  ...
  let marshal (buf : Cstruct.t) (ps : portStats) : int =
    ...
    set_ofp_port_stats_pad (Cstruct.to_string (get_new_buffer 4)) 0 buf;
...
module Error = struct
  ...
  let parse (bits : Cstruct.t) : t =
    ...
    let data = get_new_buffer (Cstruct.len err_bits) in
    (* create a new Cstruct to set the offset to 0 *)
...
module Message = struct
  ...
  let marshal (xid : xid) (msg : t) : string =
    ...
    let buf = get_new_buffer sizeof_buf in
