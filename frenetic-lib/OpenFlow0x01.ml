...
module FlowMod = struct
  ...
  let flags_to_int (check_overlap : bool) (notify_when_removed : bool) = (* XXX where is EMERG? *)
  ...
  let marshal (msg:flowMod) bits =
    ...
    set_ofp_flow_mod_buffer_id bits
      (match msg.apply_to_packet with
        | None -> -1l
        | Some bufId -> bufId);
    set_ofp_flow_mod_out_port bits (PseudoPort.marshal_optional msg.out_port); (* XXX can move to mli? *)
